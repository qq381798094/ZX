...
// 根据页码和当前页数据获取 SPU 数据
export const requestSpuDataByPageAPI = (
  page: number,
  limit: number,
  category3Id?: number,
) =>
...
export const requestSpuImgsByIdAPI = (spuId: number) =>
  request.get<any, TSpuImagesResponseData>(
    API.GET_SPU_IMAGES_BY_ID__URL + spuId,
  )

// 获取某一个 SPU 下全部的销售属性数据
export const requestSpuSaleAttributesByIdAPI = (spuId: number) =>
  request.get<any, TSpuSaleAttributesResponseData>(
    API.GET_SPU_SALE_ATTRIBUTES_BY_ID__URL + spuId,
  )

// 获取整个项目全部的销售属性属性[颜色、版本、尺码]
export const requestAllSaleAttributeAPI = () =>
  request.get<any, TAllSaleAttributesResponseData>(
